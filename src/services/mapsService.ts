...
export async function calculateRealFare(
  ...
  const calculateOptions = (distanceKm: number) => {
    const baseMultiplier = tripType === 'round' ? (2 * 0.98) : 1;
    ...
    // Round Trip Discount: 0.5% for every 500km if distance >= 500km
    // But ONLY if there are NO cancellations
    let discountMultiplier = 1;
    if (tripType === 'round' && distanceKm >= 500 && consecutiveCancellations === 0) {
      const discountPercent = Math.floor(distanceKm / 500) * 0.005;
      discountMultiplier = 1 - discountPercent;
    }

    return Object.entries(VEHICLE_RATES)
      .filter(([type]) => {
        // Auto and E-Rickshaw restricted to max 30km
        if ((type === "Auto" || type === "E-Rickshaw") && distanceKm > 30) {
          return false;
        }
        return true;
      })
