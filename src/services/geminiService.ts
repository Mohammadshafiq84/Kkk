...
export async function getRealDistance(pickup: string, dropoff: string): Promise<number> {
  try {
    const prompt = `Calculate the driving distance in kilometers between "${pickup}" and "${dropoff}" in India. 
    Return ONLY a single numeric value representing the distance in kilometers (e.g. 15.4 or 12). 
    Do not include any letters, units, words, characters, or markdown. Only output the pure number.`;
    ...
    const distanceText = response.text?.replace(/[^\d.]/g, '')?.match(/\d+(\.\d+)?/)?.[0];
    const distance = distanceText ? parseFloat(distanceText) : 12;
